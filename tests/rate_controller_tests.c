...
int main(void)
{
    ...
    rate_controller_clear_integrals(&controller);
    rate_controller_report_actuator_saturation(&controller, true);
    assert(rate_controller_process(&controller, desired, measured, 3000U, 3U,
                                   true, true, &output) ==
           RATE_CONTROLLER_RESULT_UPDATED);
    assert(controller.axis[0].integral == 0.0F);
    rate_controller_report_actuator_saturation(&controller, false);
    ...
    assert(rate_controller_process(&controller, desired, measured, 20000U, 4U,
                                   true, true, &output) ==
           RATE_CONTROLLER_RESULT_CONTINUITY_LOST);
    assert(controller.sample_seeded);
    assert(controller.axis[0].integral == 0.0F);

    assert(rate_controller_process(&controller, desired, measured, 21000U, 5U,
                                   false, false, &output) ==
           RATE_CONTROLLER_RESULT_DISABLED);
    assert(!controller.sample_seeded);
    assert(rate_controller_type_name(RATE_CONTROLLER_TYPE_PID)[0] == 'P');

    assert(rate_controller_process(&controller, desired, measured, 30000U, 6U,
                                   true, true, &output) ==
           RATE_CONTROLLER_RESULT_SEEDED);
    assert(rate_controller_process(&controller, desired, measured, 29000U, 7U,
                                   true, true, &output) ==
           RATE_CONTROLLER_RESULT_CONTINUITY_LOST);
    assert(!controller.sample_seeded);
    {
