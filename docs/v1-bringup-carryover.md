...
| --- | --- | --- |
| The equal 100 kOhm VBUS divider produces about 1 V and prevents enumeration when hardware VBUS sensing is enabled. | Implemented: the board port samples PA9 before USB initialization; V1's low level selects assume-present while a valid high selects hardware sensing. | Physically prove both runtime branches: V1 enumeration without sensing and corrected hardware with sensing. |
