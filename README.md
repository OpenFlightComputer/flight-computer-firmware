...
Phase 0, Milestone 0.11 adds a bounded newline-delimited JSON command channel to the existing non-blocking USB CDC logging path. The lowest-priority 1,000 Hz `usb-service` task frames and parses commands outside interrupt context, prioritizes pending responses, and drains at most one log record per release. `status`, basic `health`, `arm`, and `disarm` are supported; lifecycle changes go exclusively through the state machine, and `ARMED` still has no actuator effect.
