...
through 4.4 pin Bosch's official BMI270 SensorAPI, add the tester-proven V1
...
estimation, and request-driven bench visualization through
`./ofc device imu --watch`. Physical sign/timing/estimate validation and
closed-loop control remain later work; see `docs/bmi270.md` and
...
Each firmware build produces ELF, HEX, BIN, map, and compile-command artifacts. Native host tests exercise the timebase, task registry, scheduler, state transitions, faults, health derivation, logging core, USB JSON serialization, newline framing, protocol parsing, command dispatch, receiver processing and inspection, motor mapping, DShot conversion, board motor-route facts, and constrained manual motor policy. The image decodes and observes receiver input but does not yet route it to motors; sensor processing, stabilization, and operational flight behavior remain later phases.
