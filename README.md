...
Phase 1, Milestone 1.1 adds the first hardware-independent flight-layer contract: an atomic normalized four-motor command with strict finite/range validation, exact-stop canonicalization, explicit invalidation, monotonic timestamps, and bounded freshness checks. It creates no global command and produces no DShot or hardware output.
